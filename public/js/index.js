    ...
    document.querySelectorAll(".form-checkbox-resetall").forEach((button) => {
        button.onclick = () => {
            ...
            if (checkboxCotainer) {
                ...
                spans.forEach(
                    (item) => (item.innerText = item.dataset.default)
                );
            }
        };
    });
            ...
            (e, clickedIndex, isSelected, previousValue) => {
                setTimeout(() => {
                    ...
                    document
                        ...
                        .forEach((item) => {
                            item.innerText =
                                selected || item.dataset.default || "Toàn quốc";
                        });
                }, 100);
            }
        );
...
(() => {
    ...
    const removeActivedClassOfEl = (parentNode, classSelector) => {
        if (parentNode) {
            const activeNode = parentNode.querySelector(
    ...
    const rangeSlideEvent = (options) => {
        ...
        const priceRangeEl = parentNode.querySelector(
            ".range-slider-home-price"
        );
            ...
            item.onclick = () => {
                ...
                if (inputMin.value == 600) {
                    ...
                    if (inputMin.value != 500) {
                        ...
                        filterValue.innerText = formatFunction(inputMin.value);
                    }
                }
        ...
        item.onchange = () => {
            ...
            checkboxList.forEach((itemz) => {
                if (itemz !== item) {
                    // itemz.checked = false;
                    const applyBtn =
                        itemz.parentNode.querySelector(".form-close");
...
document.querySelectorAll(".addition-filter").forEach((add) => {
    ...
    add.querySelector(".dropdown-btn > ul").onchange = () => {
        const res = Array.from(
            add.querySelectorAll(".checkbox-container")
        ...
        if (count) {
            ...
            add.querySelector(".form-checkbox-resetall").addEventListener(
                "click",
                () => {
...
const runCheckAll = () => {
    const setCheckAll = (el) => {
        ...
        if (
            ul.querySelectorAll(".form-checkbox:not(:first-of-type) input")
            .length ===
            ul.querySelectorAll(
                ".form-checkbox:not(:first-of-type) input:checked"
            ).length
        ) {
    ...
    document
        ...
        .forEach((el) => {
            el.addEventListener("change", () => {
                const ul = el.closest("ul");
                ul.querySelectorAll(
                    ".form-checkbox:not(:first-of-type) input"
                ).forEach((item) => {
